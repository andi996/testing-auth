...
const UndanganInterview = ({ data, keyword, handleButton }) => {
  ...
  const optionsList = {
    ...
    source: ["ORVI", "Interview 1", "Interview 2", "Interview 3"],
  ...
  useEffect(() => {
    ...
    if (filter?.source !== null) {
      const newData = tempData?.filter((item, index) => {
        return item?.current_status?.name === filter.source;
      });
      tempData = newData;
    }
      ...
      {!Mobile && (
        <>
          ...
          <Box className="d-flex" gap="12px" m="24px 0">
            ...
            <ActionChipDropdown
              chevron="true"
              optionList={optionsList.source}
              handleClick={(val) => handleClick("source", val)}
            >
              Proses Interview
            </ActionChipDropdown>
            <ActionChipDropdown
      ...
      <Box className="d-flex-fullwidth" mb={Mobile ? `4px` : `16px`}>
        ...
        >
          <Box my="16px">
            ...
            <Box mb="16px">
              <Heading4 mb="16px"> Proses Interview</Heading4>
              <RadioButton
                type="mobile"
                variant="left"
                gap="16px"
                defaultValue={optionsList?.source?.map((item) =>
                  item === filterTemp_?.source ? true : false
                )}
                reset={resetRadio}
                options={optionsList?.source?.map((item) => {
                  return { label: item, value: item };
                })}
                onChange={(e) =>
                  setFilterTemp_((prev) => ({
...
export default UndanganInterview;
